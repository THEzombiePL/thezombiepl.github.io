...
const Ray = ({
	...
	intensity,
	direction = 'down',
}: LightRay & { direction?: 'up' | 'down' }) => {
	const isUp = direction === 'up';

	return (
		<motion.div
			className={cn(
				'pointer-events-none absolute w-(--ray-width) -translate-x-1/2 rounded-full mix-blend-screen blur-(--light-rays-blur) opacity-0 h-(--light-rays-length)',
				isUp
					? '-bottom-[12%] origin-bottom bg-linear-to-t from-[color-mix(in_srgb,var(--light-rays-color)_70%,transparent)] to-transparent'
					: '-top-[12%] origin-top bg-linear-to-b from-[color-mix(in_srgb,var(--light-rays-color)_70%,transparent)] to-transparent',
			)}
			style={
				{
					'--ray-left': `${left}%`,
